...
%         tic;
%         [x1k, f1k, gradf1k_norm, k1, x1seq,f1seq, bt1seq, taoseq1, gradfk, cos_grad] = ...
...
%     disp(['N. of success: ', num2str(sum(result_first_function)),'/10'])
...
for l = 1:1:3
    ...
    avg_roc(l) = sum(result_third_function.*conv_rate_3)/sum(result_third_function);
    avg_tao_used(l) = round(sum(result_third_function.*number_tao_3)/sum(result_third_function));
end
